define([
    ...
    var FramesPanel = function ($container, options, parentPanel) {
	parent.constructor.call(this, $container, {
	    Layout: 'Standard'
	}, parentPanel);

	this._activeFrame = null;
	this._framesoptions = options;
    };

    FramesPanel.prototype = new ParentClass();

    FramesPanel.prototype._prepare = function () {
	var self = this;
	...
	$('body').on('click.FramesPanel', function (event) {
		...
		} else if (event.which === 32 || event.which === 13) { // SPACE|
									// ENTER
		    var selection = self._selection.getSingleSelection();
		    if (selection !== false) {
	...
	$("#editframe").click(function(event) {
	    ...
	    if (!frameIdx) {
		return; // Return if no frame is selected, or multiple frames
			// are selected
	    }
	...
	app.event.bind(app.EVENT_PRESENTATION_CHANGED, function (data) {        	    
		...
		if(self._presentationEdited) {
			...
			self._loadPresentationID(data.PresentationID);
		    });        		    
		} else {
		...
		self._clearPresentation();
	    }
	});

	// If this panel has been requested with a presentation ID, set that as
	// the active presentation.
	if(this._framesoptions.PresentationID) {
	    this._loadPresentationID(this._framesoptions.PresentationID);
	}

    };

    FramesPanel.prototype._clearPresentation = function() {
	self._frames = [];
	self._updateFrames();
	self._selection.clearSelection();
	self._presentationID = undefined;
    };


    FramesPanel.prototype._loadPresentationID = function(id) {
	var self = this;
	app.db.frame
	...
	.toArray(function (frames) {
	    ...
	    self._updateFrames();
	    self._selection.clearSelection();
	    self._presentationID = id;
	});
    };

    FramesPanel.prototype._showText = function (text) {
	var text = DI.A.Text.Add({Text: text, Opacity: 0});
	text.queue(DI.A.Text.Animate(text.getID(), {Opacity: 1}));
	text.send();
    };

    FramesPanel.prototype._toggleActive = function (index) {
    ...
    FramesPanel.prototype._getActiveFrameKey = function (presentationID, index) {
	return presentationID+':'+index;
    };
    /**
     * Updates the FrameIndex value of each frame to represent its current array
     * position.
     */
    FramesPanel.prototype._updateFrameIndices = function() {
	var i = 0,
    ...
    FramesPanel.prototype._updateFrames = function () {
    ...
    FramesPanel.prototype._addNewFrame = function () {
    ...
    FramesPanel.prototype._removeFrames = function(indexes) {
	if(!($.isArray(indexes))) {
    ...
    FramesPanel.prototype._removeFrame = function (index) {
     ...
     */
    FramesPanel.prototype._saveFrames = function() {
	...
	app.db.transaction('rw', app.db.frame, function() {
		...
		for (j; j < dblength; j++) {
		    ...
		    if(!foundMatch) {
			app.db.frame.delete(dbframes[j].ID); // no match in
								// this
								// presentation,
								// must have
								// been deleted
    ...
    FramesPanel.prototype._onSaveSuccess = function() {
	console.log("Saved like a boss");
    };

    FramesPanel.prototype._onSaveFail = function(error) {
	console.log("Save failed: " + error);
    };

    FramesPanel.prototype._setPresentationEdited = function() {
    ...
    FramesPanel.prototype._shiftFrame = function(frameIdx, direction) {
	var newIndex = 0;
	...
	case 'left':
	    ...
	    break;
	case 'right':
	    if(frameIdx == frameslength - 1) return false;// already right
							    // most position
	    newIndex = frameIdx - -1; // TODO: It thinks frameIdx is a STRING
					// so 1 + 1 = 11 :( Not sure where this
					// originates.
	    break;
	default:
	    return false;
    ...
    FramesPanel.prototype._setFrameText = function (index, text) {
    ...
    FramesPanel.prototype._promptSave = function() {
	var def = Q.defer();
	var self = this;
	var options =  {
		Title : 'Save changes?',
    ...
    FramesPanel.prototype._openEditorDialog = function(frameIdx) {
	var self = this;
